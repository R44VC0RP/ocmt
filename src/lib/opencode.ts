...
async function runDeslopPrompt(
  title: string,
  prompt: string,
  modelOverride?: ModelConfig
): Promise<string> {
  const deslopModel = modelOverride ?? (await getDeslopModel());
  const { client, close } = await createIsolatedClient();
  ...
  try {
    ...
    if (!message) {
      throw new Error(
        `No response generated by ${deslopModel.providerID}/${deslopModel.modelID}. Response: ${JSON.stringify(result.data)}`
      );
    }
...
\`\`\``;
  ...
  const response = await runDeslopPrompt("oc-deslop", prompt, deslopModel);
